...
export default (props)=>{
  ...
  useEffect(()=>{
      ...
      (async()=>{
        await HttpService.post('/reportServer/bdModelTableColumn/table/getModelTableById', JSON.stringify({table_id:path2[0].slice(1)})).then(res => {
          if (res.resultCode == "1000") {
                    ...
                    setnotes(res.data.table.table_desc)
    ...
    const  baocun =()=>{
      ...
      console.log(mypath)
      HttpService.post('/reportServer/bdModelTableColumn/table/createModelTable', JSON.stringify({model_id:mypath[0]!=="X"?mypath:mypath[1],table_name:formName,table_desc:notes,table_id:mypath[0]!=="X"?"":mypath[0].slice(1),columnlist:[...tableData],linkList:[...tableData2],deleteColumnList:[],deleteTableLinkList:[],dbtype_id:model.db_type,source_id:"",host_id:model.db_source,url:'url',catalog_id:"1"})).then(res => {
        if (res.resultCode == "1000") {   
